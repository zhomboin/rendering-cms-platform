...
grep -q 'APP_ENV: ${APP_ENV:-development}' "$COMPOSE_FILE" || fail "backend service must default to development app env"
grep -q 'DEV_BOOTSTRAP_ADMIN: ${DEV_BOOTSTRAP_ADMIN:-true}' "$COMPOSE_FILE" || fail "backend service must bootstrap a dev admin by default"
grep -q 'DEV_ADMIN_EMAIL: ${DEV_ADMIN_EMAIL:-admin@rendering.me}' "$COMPOSE_FILE" || fail "backend service must configure default dev admin email"
grep -q 'DEV_ADMIN_PASSWORD: ${DEV_ADMIN_PASSWORD:-rendering_dev_password}' "$COMPOSE_FILE" || fail "backend service must configure default dev admin password"
...
grep -q 'S3_PUBLIC_BASE_URL: ${S3_PUBLIC_BASE_URL:-http://127.0.0.1:9000/rendering-assets}' "$COMPOSE_FILE" || fail "backend service must expose local object public base URL"
...
grep -q '^S3_PUBLIC_BASE_URL=http://127.0.0.1:9000/rendering-assets$' "$TMP_DIR/.env" || fail "sync script should default public object base URL to localhost bucket path"
