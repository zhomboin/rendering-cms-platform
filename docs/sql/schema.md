...
- `articles`：文章主表，保存系统生成的 6 位短链码 `slug`、用户命名的英文名 `article_name`、标题、摘要、MDX 正文、状态、标签、精选标记、封面地址、发布时间、作者、版本号和全文搜索向量。
...
- `analytics_events`：可选访问事件明细表，保存事件类型、文章关联、IP 哈希、User-Agent 和创建时间，用于后续更细粒度分析。
...
- `analytics_events` 是增强阶段的可选明细基础，不替代 daily/history 聚合表；后台趋势接口默认读取聚合表。
