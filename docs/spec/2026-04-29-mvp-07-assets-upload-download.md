...
- [ ] 在 `service_test.go` 编写 `TestValidateUpload`。
- [ ] 在 `service.go` 定义 `MaxUploadBytes = 20 * 1024 * 1024`。
- [ ] 在 `service.go` 定义允许类型：`image/png`、`image/jpeg`、`image/webp`、`application/pdf`、`text/plain`、`application/zip`。
- [ ] 在 `service.go` 实现 `ValidateUpload(filename, contentType string, byteSize int) error`。
- [ ] 在 `storage/s3.go` 封装 S3 兼容客户端创建逻辑。
- [ ] 在 `storage/s3.go` 实现预签名上传 URL 方法。
- [ ] 在 `storage/s3.go` 实现预签名下载 URL 方法。
- [ ] 在 `backend/sql/assets.sql` 增加创建 `assets` 记录 SQL。
- [ ] 在 `backend/sql/assets.sql` 增加查询 `assets` 记录 SQL。
- [ ] 在 `backend/sql/assets.sql` 增加创建 `download_events` 记录 SQL。
- [ ] 在 `handler.go` 实现 `POST /api/v1/admin/assets/upload-url`。
- [ ] 在 `handler.go` 实现 `GET /api/v1/admin/assets/{id}/download-url`。
- [ ] 创建 `AdminAssetsPage.tsx`，包含上传表单、资源列表、下载按钮区域。
- [ ] 创建 `docs/apis/assets.md`。
- [ ] 运行 `cd backend && go test ./internal/assets`。
