...
实现供 Rendering 博客读取的已发布文章接口、后台草稿编辑、发布和版本日志基础能力，并提供从原静态博客 MDX 文件导入文章的工具入口。
...
- [x] 在 `backend/sql/articles.sql` 增加 Rendering 博客文章列表 SQL，只返回 `published`。
- [x] 在 `backend/sql/articles.sql` 增加 Rendering 博客文章详情 SQL，只返回 `published`。
...
- [x] 创建 `docs/apis/articles.md`，记录 Rendering 博客内容读取接口和后台文章 API。
...
- 完成时间：2026-04-30。
- Rendering 博客内容读取接口只读取 `published` 状态文章。
